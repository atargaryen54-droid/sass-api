...
class BillingService:
    ...
    def calculate_usage(db: Session, project_id: int):
        ...
        results = (
            ...
            .outerjoin(
                PricingRule,
                UsageEvent.event_type_id == PricingRule.event_type_id,
            )
        ...
        for row in results:
            unit_price = (
                row.price_per_unit if row.price_per_unit is not None else 0.0
            )
    ...
    def generate_project_billing(db: Session, project: Project):
        try:
            ...
            BillingService.generate_invoices(
                db=db,
